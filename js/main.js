document.addEventListener('DOMContentLoaded', function () {
  ...
  const renderChinhTriGia = (data) => {
    const pageChinhTriGia = document.getElementById('pageChinhtrigia')
    console.log('pageChinhTriGia')
    const chinhTrigiaProfiles = filterProfilesByType(data, 'chinhtrigia')
      ...
      .then((data) => {
        ...
        if (profile) {
          ...
          profileContent.querySelector(
            '.profile-hometown'
          ).innerHTML = `<i class="fas fa-location-arrow">${profile.thongtincanhan.quequan}</i>`
          profileContent.querySelector(
            '.profile-education'
          ).innerHTML = `<i class="fas fa-user-graduate">${profile.thongtincanhan.hocvi}</i> `
          //
